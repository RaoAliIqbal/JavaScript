...
function start(){
   ...
   for(let i=0;i<allBox.length;i++){
       ...
       allBox[i].style.width=(containerWidth/items)-margin + "px";   // 1333/3 = 444
       allBox[i].style.margin=(margin/2)+ "px";
      totalItemsWidth+=containerWidth/items;   //   444 x 10    (1333/3 = 444)
      totalItems++;
   }
   ...
   // slides controls number set up
    const allSlides=Math.ceil(totalItems/items);  // 10/3 = 4
...
function controlSlides(arg){
    ...
    const li=ul[0].children
     ...
     arg.className="active";  //arg=li#2.active

     var numb=(arg.id-1)-active;  // (2-1)-0
